...
std::vector<double> interpolateVisToImu(const std::vector<VisSample>& vis,
    ...
    // 遍历每个IMU时间戳t
    for (double t : imu_t) {
        // 根据t_cam+t_bias=t_imu逻辑把imu和视觉信息先统一到视觉时间尺度
...
int main(int argc, char** argv) {
    // 初始化ROS节点
    ros::init(argc, argv, "camera_imu_portion_node");
    // 创建节点句柄
    ros::NodeHandle nh;
    // 订阅
    ...
    ROS_INFO("camera_imu_portion_node started. Waiting for images and imu...");
    ros::spin();
    return 0;
}
