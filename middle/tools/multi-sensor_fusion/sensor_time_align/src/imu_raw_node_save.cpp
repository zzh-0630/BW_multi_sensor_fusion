...
#define CSV_SAVE_PATH   "/home/cat/1230/imu_data.csv"
...
static int create_dir_if_not_exist(const char *dir_path);  // 创建目录函数
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        ...
        ROS_ERROR("Usage: rosrun sensor_time_align %s /dev/i2c-6", argv[0]);
    ...
    // 以w模式打开CSV文件并写入表头。w模式：没有则创建，有则覆盖。
...
// 工具函数
// 路径创建函数create_dir_if_not_exist：如果不存在路径的话创造路径
...
// 高精度时间戳获取函数get_highres_timestamp：获取微秒级高精度时间戳
...
// MPU6050初始化函数mpu6050_init：实现MPU6050的初始化
static int mpu6050_init(int i2c_fd, uint8_t dev_addr)
{
    // 唤醒设备：PWR_MGMT_1写0x00，因为默认0x40是休眠状态，必须唤醒才能工作
...
// I2C写寄存器函数i2c_reg_write：给指定的寄存器写入数值
...
// I2C读寄存器函数i2c_reg_read：从指定寄存器读出数值
...
// 传感器数据读取函数sensor_data_read：合并高8位和低8位数据从而读取16位传感器数据
