...
#include <sensor_msgs/LaserScan.h>    // 2D激光雷达消息类型，接收雷达数据
...
struct LidarSample {
    double t;                            // 雷达时间戳
    std::vector<cv::Point2f> pts;        // 有效扫描点的直角坐标
};
...
void lidarPolarToCartesian(const sensor_msgs::LaserScan::ConstPtr& msg,
    ...
    for (float r : msg->ranges) {
        // 过滤无效点
        if (std::isinf(r) || std::isnan(r) || r < LIDAR_MIN_RANGE || r > LIDAR_MAX_RANGE) {
            // 角度自增
...
std::vector<double> computeLidarMotionFeature(const std::vector<LidarSample>& lidar_seg) {
    ...
    for (size_t i = 0; i + 1 < lidar_seg.size(); ++i) {
        ...
        double dx = c2.x - c1.x;
        double dy = c2.y - c1.y;
        motion.push_back(std::sqrt(dx*dx + dy*dy));
    }
...
void lidarCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
    ...
    // 雷达时间戳修正
    out_msg->header.stamp = ros::Time(s.t + estimated_lidar_bias);
    // 发布修正后的雷达数据
...
int main(int argc, char** argv) {
    // 节点的初始化
    ros::init(argc, argv, "lidar_imu_distance_node");
    ...
    // 创建发布者，发布修正后雷达话题
    pub_lidar_correct = nh.advertise<sensor_msgs::LaserScan>(output_topic, 1000);
