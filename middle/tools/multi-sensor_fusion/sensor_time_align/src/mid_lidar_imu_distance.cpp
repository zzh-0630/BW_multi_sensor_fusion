...
struct LidarSample {
    double t;                                 // 雷达帧级时间戳
    std::vector<cv::Point3f> pts;             // 有效扫描点的3D直角坐标x/y/z
...
// computeLidarMotionFeature函数，计算雷达帧间重心3D偏移距离
std::vector<double> computeLidarMotionFeature(const std::vector<LidarSample>& lidar_seg) {
    ...
    for (size_t i = 0; i + 1 < lidar_seg.size(); ++i) {
        ...
        // 计算两帧重心的3D偏移距离trans_distance
        double dx = c2.x - c1.x;
        double dy = c2.y - c1.y;
        double dz = c2.z - c1.z;
        double trans_distance = std::sqrt(dx*dx + dy*dy + dz*dz);
        
        // 直接将重心3D偏移距离trans_distance作为运动特征
        motion.push_back(trans_distance);
    }
