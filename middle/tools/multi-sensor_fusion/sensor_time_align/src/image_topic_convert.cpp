...
private:
    ...
    // 发布sensor_msgs/Image类型的目标话题/image_timestamp_raw
...
public:
    ...
    ImageTimestampPublisher() : first_frame_published_(false) {
        // 初始化发布者：话题名/image_timestamp_raw，消息类型sensor_msgs/Image，队列大小10
        ...
        ROS_INFO_STREAM("Subscribed Topic: /camera/color/image_raw");
    ...
    void waitForCameraTopic(double timeout) {
        ROS_INFO_STREAM("Waiting for Camera Topic '/camera/color/image_raw' (Timeout: " << timeout << "s)...");
        ...
        while (ros::ok()) {
            ...
            if (ros::master::getTopics(topic_info_list)) {
                for (const auto& topic_info : topic_info_list) {
                    if (topic_info.name == "/camera/color/image_raw") {
    ...
    void imageCallback(const sensor_msgs::ImageConstPtr& msg) {
        ...
        timestamp = round(timestamp * 1e6) / 1e6;                      // 精确到微秒
        ...
        cv_ptr_->header.frame_id = "camera_color_optical_frame";       // 设置坐标系ID

        // 发布sensor_msgs/Image类型消息到/image_timestamp_raw话题
...
int main(int argc, char** argv) {
    // 初始化ros节点
    ros::init(argc, argv, "image_topic_convert_node");
